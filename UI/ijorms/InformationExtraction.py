...
def extractSkills(skills):
    """
    Extract proper and relevant skills and create ontology
    :param skills: extracted skills from the resume
    :return: skills keywords only, removes unnecessary and redundant words
    """
    IEskills = []
    news = []
    for i in skills:
        ...
        new = []
        p = nltk.pos_tag(i)
        # Skills are generally nouns, so extract nouns
        for j in range(len(p)):
            if p[j][1] == 'NN' or p[j][1] == 'NNS' or p[j][1] == 'NNP' or p[j][1] == 'NNPS' or p[j][1] == 'JJ':
                IEskill.append(p[j][0])
                # Get multiple worded nouns like Software Development (Just an example)
    ...
    for i in IEskills:
        ...
        for j in i:
            ...
            for iii in range(len(tree)):
                ...
                if (len(data) != 0):
                    # See if the extracted skill is a proper and recognized skill
    ...
    return IEs, ontologySkill


# Extract workexperience similar to extractSkills
def extractWorkExperience(workExperience):
    """
    Extract proper and relevant work experience from extracted workexperience, also create ontology
    :param workExperience: extracted workexperience
    :return: proper and recognized work experience only
    """
    IEWorkExperiences = []
    news = []
...
def extractCertification(certification):
    """
    Extracts relevant and proper certifications from extracted certification, and also creates ontology
    :param certification: extracted certifications
    :return: proper and recognized certifications
    """
    seed = ["certification"]
    finals = []
    links = []
